---
name: saude-alimentacao
description: Saúde e alimentação — comida, bebida, hospital, doenças, cura, necessidades fisiológicas. Use para perguntas sobre comer, beber, ferimentos tratados em hospital, infecção, fome, sede ou cansaço.
---

# Saude Alimentacao (Dwarf Fortress — fortress-mode)

Localize e leia **apenas** o artigo relevante em `references/` (progressive disclosure — não leia tudo).

## Como buscar
Se não souber o arquivo exato, rode a busca local (a partir da raiz `df-skills/`):
`bash scripts/search.sh fortress-mode/saude-alimentacao "termo de busca"`
e leia o arquivo retornado.

## Índice de artigos (71)
- Alcohol → `references/alcohol.md`
- Alcohol plant stockpile → `references/alcohol-plant-stockpile.md`
- Ash → `references/ash.md`
- Bone doctor → `references/bone-doctor.md`
- Buckwheat → `references/buckwheat.md`
- Cave wheat → `references/cave-wheat.md`
- Cheating → `references/cheating.md`
- Cheese → `references/cheese.md`
- Chief medical dwarf → `references/chief-medical-dwarf.md`
- Create new FAQ category → `references/create-new-faq-category.md`
- Creature flags in DF memory → `references/creature-flags-in-df-memory.md`
- Creature/Edit notice → `references/creature-edit-notice.md`
- Creatures → `references/creatures.md`
- Crutch → `references/crutch.md`
- Diagnostician → `references/diagnostician.md`
- Doctor → `references/doctor.md`
- Dwarf cancels construction of Wall: Creature occupying site → `references/dwarf-cancels-construction-of-wall-creature-occupying-site.md`
- Dwarf cancels task: Handling dangerous creature → `references/dwarf-cancels-task-handling-dangerous-creature.md`
- Dwarf cancels task: Hunting vermin for food → `references/dwarf-cancels-task-hunting-vermin-for-food.md`
- Dwarven milk → `references/dwarven-milk.md`
- Dwarven sugar → `references/dwarven-sugar.md`
- Dwarven syrup → `references/dwarven-syrup.md`
- Egg → `references/egg.md`
- Egg production → `references/egg-production.md`
- Farm size calculations → `references/farm-size-calculations.md`
- Farmer's workshop → `references/farmer-s-workshop.md`
- Farming → `references/farming.md`
- Flour → `references/flour.md`
- Food → `references/food.md`
- Food hauling → `references/food-hauling.md`
- Forest retreat → `references/forest-retreat.md`
- Gypsum plaster → `references/gypsum-plaster.md`
- Hard wheat → `references/hard-wheat.md`
- Health care → `references/health-care.md`
- Health screen → `references/health-screen.md`
- Honey → `references/honey.md`
- How do you create a custom stockpile that accepts only prepared foods? → `references/how-do-you-create-a-custom-stockpile-that-accepts-only-prepared-foods.md`
- Irrigation → `references/irrigation.md`
- Kitchen → `references/kitchen.md`
- Language SYM.txt/Death → `references/language-sym-txt-death.md`
- Language SYM.txt/Food → `references/language-sym-txt-food.md`
- Leather works → `references/leather-works.md`
- Leatherworker → `references/leatherworker.md`
- List of creatures by adult size → `references/list-of-creatures-by-adult-size.md`
- List of creatures by pet value → `references/list-of-creatures-by-pet-value.md`
- Meal → `references/meal.md`
- Meat → `references/meat.md`
- Milk → `references/milk.md`
- Missing features → `references/missing-features.md`
- Mog juice → `references/mog-juice.md`
- Night creature → `references/night-creature.md`
- Obsidian farming → `references/obsidian-farming.md`
- Oil → `references/oil.md`
- Peat → `references/peat.md`
- Potash → `references/potash.md`
- Prepared organs → `references/prepared-organs.md`
- Press cake → `references/press-cake.md`
- Royal jelly → `references/royal-jelly.md`
- Shrub → `references/shrub.md`
- Single-grain wheat → `references/single-grain-wheat.md`
- Soap → `references/soap.md`
- Soft wheat → `references/soft-wheat.md`
- Surgeon → `references/surgeon.md`
- Suturer → `references/suturer.md`
- Symptom → `references/symptom.md`
- Tallow → `references/tallow.md`
- Tree farming → `references/tree-farming.md`
- Two-grain wheat → `references/two-grain-wheat.md`
- White mountain heather → `references/white-mountain-heather.md`
- Wound → `references/wound.md`
- Wound dresser → `references/wound-dresser.md`
