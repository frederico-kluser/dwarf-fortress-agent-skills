...
> Fonte: [Interface.txt](https://dwarffortresswiki.org/index.php/Interface.txt) — Dwarf Fortress Wiki (GFDL/MIT)
...
The game key bindings are stored in **interface.txt**. They can be configured in the section of the in-game options screen or by editing this file directly.
...
When in-game, press to open the options screen and select . This shows the various game mode/screen categories (and Macros).
...
For example, `[SYM:0:A]` represents , `[SYM:1:Enter]` represents -, and `[SYM:6:Left]` represents --.
...
`KEY` is a simpler version of `SYM`, only allowing plain keys (no modifiers). These are listed as in the Key Bindings menu.

Syntax:

    [KEY:Key]

For example, `[KEY:a]` represents the key.
