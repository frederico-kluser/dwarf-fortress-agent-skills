...
Books containing "secrets of life and death" are occasionally written during world generation by necromancers and various demonic rulers. Any mortal creature who reads one will immediately learn the secrets to immortality and the ability to raise the dead. Necromancy books can be found in a necromancer's tower "tower") among other types of books, including the original slab which the secrets originated from. A book will state it "concerns the secrets of life and death" if it does, note that books with descriptions like "Concerns the learning of the secrets of life and death" do *not* contain the secret and as such cannot train necromancers.
...
- Codex/Codices/Scroll can be stuck on the map, and not be interacted with (from raid or visitor), which can be fixed with DFHack.

## See Also

- Text_book_instruction.txt

 Ru:Book Fr:Livres (books) "Fr:Livres (books)")
