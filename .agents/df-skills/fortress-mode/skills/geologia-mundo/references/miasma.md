...
> Fonte: [Miasma](https://dwarffortresswiki.org/index.php/Miasma) — Dwarf Fortress Wiki (GFDL/MIT)

**Miasma** is a cloud of purple stench that makes your dwarves unhappy. It only occurs in subterranean areas - (specifically, tiles; whether a tile is or makes no difference), and is caused by rotting items. It does not pass through closed doors.

Miasma completely blocks the view of things, even in its most dispersed form (though this doesn't interfere with -looking). However, it is planned to have it be translucent to a certain degree in the premium release. 1 Miasma is not shown in Adventurer mode, although it will be listed when ooking, and can cause negative thoughts.
...
Often a job in a kitchen, still or farmer's workshop will be canceled with the announcement . That doesn't mean you only have rotten ones, it means there was not an appropriate item available at that moment. Items that are involved in another task, forbidden, marked for dumping, or that have no path will be disregarded for workshop tasks.
...
To avoid miasma, either keep anything rotten above ground, submerged in 7/7 water, or alternatively have a subterranean refuse pile on its own in a room with several doors; this should halt the miasma's advance (although the room itself will still stink up). It will also not spread diagonally, so one possible solution is to have the only entrance be diagonal.
