...
> Fonte: [Nest box](https://dwarffortresswiki.org/index.php/Nest_box) — Dwarf Fortress Wiki (GFDL/MIT)
...
Once created, the nest box is then uilt as a Wrkshop of type armer as a est box, and will not block movement. The box will then be claimed by an egg-laying animal when it is ready to lay eggs. The animal that claims the nest box will periodically lay eggs into the nest. This process is very quick (\<1 day) and the eggs are ready to be collected immediately if they are to be cooked, but require time to hatch if you wish to start a breeding program.
...
3.  Create a 1×1 pasture zone on top of the nest box, and assign one female egg-layer to it. She should claim the nest box when she is ready to lay eggs.
...
Elk birds are both grazers and egg layers. Females left undisturbed near a nest box will probably starve themselves to death (certainly, if there are no males available). A possible workaround is to assign the bird a pasture close to the nest, but not including it, so that dwarves with Animal hauling on would pasture the bird now and then, then she would return. Eggs do not require continuous presence, only return before timeout ().

Aquatic creatures (such as sea serpents) refuse to use a submerged nest box, which makes breeding impossible.
