...
> Fonte: [Hive](https://dwarffortresswiki.org/index.php/Hive) — Dwarf Fortress Wiki (GFDL/MIT)

A **hive** is a tool and building that stores honey bee colonies. It is used in the beekeeping industry for the production of honeycombs and royal jelly, which can be processed into usable items. Citizens with the beekeeping labor enabled handle the building, removal, and management of hives.

Hives use the same character as logs. As an item, they have the color of their material, but built ones are displayed as yellow.
...
Hives are built by beekeepers from the uilding menu, using as a shortcut. For hives to produce honey, they must be built adjacent to (or on) an above ground tile. Subterranean hives can still store colonies but will not make products.

Removing a building () returns the hive and any items inside it, including the colony, which is released as a single honey bee that eventually disappears if left outside. Removed honeycombs can be collected and hauled, but any royal jelly is spilled onto the ground and wasted.

## Settings

The hive's settings can be found by opening the set building tasks/prefs menu () and hovering over a built hive. There are two options which control beekeepers from doing certain jobs with the hive.

toggles whether beekeepers can install new colonies into the hive. It is set to true () by default. Changing the settings to will not allow the hive to accept new colonies. Preexisting colonies in the hive are not affected.

toggles whether beekeepers can gather honeycombs and royal jelly from the hive. It is also set to true () by default. Changing the settings to will prevent beekeepers from gathering the products inside the hive, leaving the colonies alive so they can be saved for splitting.
...
Items in the hive can be seen in the "view iems in buildings" menu. Each hive can store a colony, which is represented as a stack of live honey bees. Colonies are collected from the wild or by splitting colonies from other hives. Three months after installation, colonies become , which allows them to be used to install other hives. It produces one honeycomb and royal jelly after six months after installation, unless the colony count limit was exceeded. Once both items are produced, a beekeeper with an empty jug goes to gather the items, storing the royal jelly in the jug. Beekeepers leave both the freed honeycomb and jug of royal jelly in the hive, and food haulers carry them to a stockpile. When a hive's products are gathered, the colony is destroyed, and the hive requires a new colony in order to produce again.
