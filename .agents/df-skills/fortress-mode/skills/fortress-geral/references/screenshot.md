...
> Fonte: [Screenshot](https://dwarffortresswiki.org/index.php/Screenshot) — Dwarf Fortress Wiki (GFDL/MIT)
...
2.  Press + to capture the active window. The image will be placed in your operating system clipboard.
    - If you want to capture the whole screen, press instead.
3.  Open your image editor (usually MSPaint).
4.  Press + to paste the saved screenshot into paint.
...
5.  Under \, select \, and save your image as a .PNG or .JPG file. For standard tilesets, PNG files are preferred as they tend to be smaller and do not lose any detail during compression.
...
5.  You can now press +, or click the save button in the top right of the new window to save the screenshot in a location of your choosing. It defaults to .PNG format.

### MacOS

1.  Press ++; the mouse cursor will turn into a crosshair.
2.  Press to turn the mouse cursor into a camera icon.
...
3.  Add the name in double brackets, the same as creating any wiki active link, with the word "Image:", at the front of the image file name. Using just the file name gives the image as it was saved, so adding to a wiki page gives this:\
...
Pro Tip: *First*, make a copy of the image and give *that* the new name - then open and edit *that* file. Then, when you hit  + out of habit, you'll be saving to that file name, not over the original. \]\]
