---
name: df-adventure-mode
description: Guia de conhecimento do Dwarf Fortress para o Adventure Mode. Foco em jogar como um aventureiro: criação de personagem, viagem, exploração de sítios, conversas e combate em primeira pessoa. Roteia a pergunta do jogador para a skill de categoria certa (combate, criaturas, materiais, etc.), carrega o SKILL.md dela e lê o artigo específico em references/.
---

# Dwarf Fortress — Adventure Mode (Roteador)

Você está no **Adventure Mode**. **Não responda de memória** sobre mecânicas — roteie para a categoria certa, carregue o `SKILL.md` dela e leia o arquivo de referência específico (progressive disclosure).
...
| criação de personagem, viagem, fast travel, conversas, acampamento, sítios, retire/reclaim | `skills/adventure-geral` (78 artigos) |
...
`bash scripts/search.sh adventure-mode "termo de busca"`
ou restrinja a uma categoria:
`bash scripts/search.sh adventure-mode/combate "termo"`
