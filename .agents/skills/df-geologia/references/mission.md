...
Dwarves counting their raided spoils.\
*Art by steamingcore*
...
- A Raid uses the Ambusher skill, probably checking the average of all dwarves on the mission\[Verify\] against the defending site leader's Observer skill.[1]
...
- When a squad returns from a mission, an announcement to the effect of " has returned" will be generated, and a mission report will be visible in the report menu accessible from the world screen. In the report, the path the squad took is traced on the map, and the events that took place along the way are revealed.
