...
> Fonte: [Visitor](https://dwarffortresswiki.org/index.php/Visitor) — Dwarf Fortress Wiki (GFDL & MIT). Snapshot 2026-06.
...
Visitor population cap is set to 100 by default, can be modified in settings, and does not count long-term residents; if set to 0, visitors will not come at all.
...
Monster slayers and beast hunters seek wild creatures to kill, or just a tavern for relaxing and socializingv0.44.01. Monster slayers start arriving after a cavern is revealed; there does not need to be an unrestricted/public location, such as a tavern - the discovery of a cavern is enough.
...
Monster slayers go down into the caverns alone to slay subterranean creatures. Beast hunters remain above-ground and hunt wild animals, similar to hunters.\[Verify\]. Neither can have any labor enabled, but they are another able (and expendable) body if something unpleasant gets to where they are.
...
When accepted, visitors will be added to the citizens list in the units screen. Their needs, preferences, and thoughts will be visible as normal, but labors or occupations cannot be assigned to them. Soldiering visitors can be assigned to squads, but they cannot be appointed as Militia captains. Aside from doing their main activities, long-term residents eat, drink, sleep, perform other activities, and reside at the Tavern if rooms are available. Locations set to Citizens and long-term residents only are allowed for long-term residents; locations set to Citizens only, however, disallow long-term residents.
