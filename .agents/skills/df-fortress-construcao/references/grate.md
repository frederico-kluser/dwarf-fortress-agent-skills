...
> Fonte: [Grate](https://dwarffortresswiki.org/index.php/Grate) — Dwarf Fortress Wiki (GFDL & MIT). Snapshot 2026-06.
...
Unlike a constructed wall, grates have quality levels and can be decorated. They can be constructed with bnG.

## Floor grate

Where the unwanted liquids go.

- Note: Placing a floor gate has some important considerations - see below!
...
Unlike wall grates, floor grates block projectiles. They do not block line of sight. Floor grates can be linked to a lever or a pressure plate. When triggered, a floor grate opens like a hatch, but with a 100-tick delay. They can be constructed with bng
...
Floor grates must be attached orthogonally (i.e. not diagonally) to firm ground or some solid construction (i.e. a wall, floor, or stair.), not (just) other grates. Floor grates placed directly on top of a floor or staircase still require orthogonal support, even though intuitively their own tile should be able to provide support - it does not. Attempting to construct a floor grate supported only by its own tile will display the message "Surrounded by empty space". Removing the orthogonal support from a constructed floor grate will cause it to harmlessly deconstruct.

- **WARNING**: A floor grate does NOT count as solid construction. It will not support any constructions (i.e. walls, floors or stairs) nor any buildings. Annoyingly, you *can* designate a construction to be built orthogonally to a grate despite no other supports, and your dwarves will obediently carry out the order. *This will cause the construction to cave-in immediately after being completed.* Note that this can be useful for controlled cave-ins.
